...
echo ========================================
echo Starting DocTrack EDMS (Production Mode)
echo ========================================

echo Building frontend assets...
call npm run build
if errorlevel 1 (
  echo [ERROR] Frontend build failed. Stopping startup.
  pause
  exit /b 1
)
...
if exist ".venv\Scripts\activate.bat" (
  ...
  start "DocTrack Backend (Prod)" cmd /k "call .venv\Scripts\activate.bat && set PADDLE_PDX_DISABLE_MODEL_SOURCE_CHECK=True && set DOCTRACK_BACKEND_HOST=%DOCTRACK_BACKEND_HOST% && set DOCTRACK_BACKEND_PORT=%DOCTRACK_BACKEND_PORT% && set DOCTRACK_OCR_TARGET_PAGE=%DOCTRACK_OCR_TARGET_PAGE% && cd /d server && python run_production.py"
) else (
  echo [ERROR] Required environment .venv was not found.
  echo Please create or restore .venv, then run start_prod.bat again.
  pause
  exit /b 1
)
...
echo Starting frontend static preview server...
if /I "%DOCTRACK_FRONTEND_HTTPS%"=="1" (
  start "DocTrack Frontend (Prod)" cmd /k "set DOCTRACK_HTTPS=1 && npm run preview -- --host 0.0.0.0 --port 3000 --strictPort"
) else (
  start "DocTrack Frontend (Prod)" cmd /k "npm run preview -- --host 0.0.0.0 --port 3000 --strictPort"
)

echo.
echo Production services started in separate windows.
echo Local URL: %DOCTRACK_FRONTEND_SCHEME%://localhost:3000
echo LAN URL:   %DOCTRACK_FRONTEND_SCHEME%://YOUR-PC-IP:3000
...
echo.
echo NOTE: Close those two windows to stop the services.
pause
